...
//					+ " door " + IndoorSpace.iDoors.size());
	...
	class PaintingPanel extends JPanel {
		...
		private void paintIdrObjs(Graphics g) {
			...
			Iterator<TrajectoryPoint> itr3 = IndoorSpace.iTraPoint.iterator();
			while (itr3.hasNext()) {
				TrajectoryPoint curIdrObj = itr3.next();
//				if( curIdrObj.getTraPointId() > 195|| curIdrObj.getTraPointId() < 188) continue;
				g.fillOval(
						(int) (curIdrObj.getX() * DataGenConstant.zoomLevel),
						(int) (curIdrObj.getY() * DataGenConstant.zoomLevel),
						5, 5);
			}
		}
