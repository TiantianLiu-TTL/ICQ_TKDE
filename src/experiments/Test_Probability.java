...
public class Test_Probability {
    ...
    public static String result_pre_probability_icq_naive = System.getProperty("user.dir") + "/result/result_" + dataset + "_pre_probability_icqNaive.txt";
...
//    public static String result_pre_probability_icq_base = System.getProperty("user.dir") + "/result/result_" + dataset + "_pre_probability_icqBase.txt";
    ...
    public static void run(String outFileTime, String outFileMemory) throws IOException {
        String resultTime = "probability" + "\t" + "icq_naive" +  "\t" + "icq" + "\t" + "icq_baseNew" + "\t" + "icq_baseUnseen" + "\n";
        String resultMemo = "probability" + "\t" + "icq_naive" + "\t" + "icq" + "\t" + "icq_baseNew" + "\t" + "icq_baseUnseen" + "\n";
        ...
        for (int i = 0; i < probabilitys.size(); i++) {
            ...
            s0 += probabilityThreshold;
            s1 += probabilityThreshold;
            s2 += probabilityThreshold;
            s3 += probabilityThreshold;
...
//                    Algo_ICQ_baseline_unseen.icq_baseline_unseen_clear();
        ...
        try {
            FileWriter output = new FileWriter(result_pre_probability_icq_naive);
            output.write(s0);
            output.flush();
            output.close();
        } catch (IOException e) {
        ...
        try {
            FileWriter output = new FileWriter(result_pre_probability_icq);
            output.write(s1);
            output.flush();
            output.close();
        } catch (IOException e) {
        ...
        try {
            FileWriter output = new FileWriter(result_pre_probability_icq_base_new);
            output.write(s2);
            output.flush();
            output.close();
        } catch (IOException e) {
        ...
        try {
            FileWriter output = new FileWriter(result_pre_probability_icq_base_unseen);
            output.write(s3);
            output.flush();
            output.close();
        } catch (IOException e) {
...
//        PreCompute.preInfo_read(dataset, 4000, DataGenConstant.sample_dist);
        ...
        // probability
        String outFileTime = System.getProperty("user.dir") + "/result/" + dataset + "_probability_time.csv";
        String outFileMemory = System.getProperty("user.dir") + "/result/" + dataset + "_probability_memory.csv";
