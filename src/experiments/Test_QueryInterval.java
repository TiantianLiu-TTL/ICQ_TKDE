...
public class Test_QueryInterval {
    public static String dataset = "syn";
    public static double probabilityThreshold = 0.5;

    public static String result_pre_queryInterval_icq_naive = System.getProperty("user.dir") + "/result/result_" + dataset + "_pre_queryInterval_icqNaive.txt";
    ...
    public static void run(String outFileTime, String outFileMemory) throws IOException {
        String resultTime = "queryInterval" + "\t" + "icq_naive" +  "\t" + "icq" + "\t" + "icq_baseNew" + "\t" + "icq_baseUnseen" + "\n";
        String resultMemo = "queryInterval" + "\t" + "icq_naive" + "\t" + "icq" + "\t" + "icq_baseNew" + "\t" + "icq_baseUnseen" + "\n";
        ...
        for (int i = 0; i < endTimes.size(); i++) {
            ...
            System.out.println("endTime: " + DataGenConstant.endTime);

            resultTime += DataGenConstant.endTime / 3600 + "\t";
            resultMemo += DataGenConstant.endTime / 3600 + "\t";

            s0 += DataGenConstant.endTime / 3600;
            s1 += DataGenConstant.endTime / 3600;
            s2 += DataGenConstant.endTime / 3600;
            s3 += DataGenConstant.endTime / 3600;
...
//                    Algo_ICQ_baseline_unseen.icq_baseline_unseen_clear();
        ...
        try {
            FileWriter output = new FileWriter(result_pre_queryInterval_icq_naive);
            output.write(s0);
            output.flush();
            output.close();
        } catch (IOException e) {
        ...
        try {
            FileWriter output = new FileWriter(result_pre_queryInterval_icq);
            output.write(s1);
            output.flush();
            output.close();
        } catch (IOException e) {
        ...
        try {
            FileWriter output = new FileWriter(result_pre_queryInterval_icq_base_new);
            output.write(s2);
            output.flush();
            output.close();
        } catch (IOException e) {
        ...
        try {
            FileWriter output = new FileWriter(result_pre_queryInterval_icq_base_unseen);
            output.write(s3);
            output.flush();
            output.close();
        } catch (IOException e) {
...
//        PreCompute.preInfo_read("syn", 4000, DataGenConstant.sample_dist);
        ...
        // queryInterval
        String outFileTime = System.getProperty("user.dir") + "/result/" + dataset + "_queryInterval_time.csv";
        String outFileMemory = System.getProperty("user.dir") + "/result/" + dataset + "_queryInterval_memory.csv";
