...
//                System.out.println("something wrong with D2PLeave size: dk.aau.cs.indoorqueries.algorithm.IDModel_SPQ.pt2ptDistance3");
            ...
            if (tempParId != -1) {
                ...
                if (tempLeaveDoors.equals(new ArrayList<Integer>(Arrays.asList(tempDoorId))) && tempParId != tPartitionId) {
                    sDoors.remove(i);
                    i--;
                }
            }
        ...
        for (int i = 0; i < sDoors.size(); i++) {
            ...
            for (int j = 0; j < IndoorSpace.iDoors.size(); j++) {
                if (j != sDoorId) {
                    ...
                    dist[j] = 0;
                    prev.add(new ArrayList<Integer>(Arrays.asList(sPartitionId, -1)));
                }
                H.insert(dist[j], j);
...
//                                System.out.println("dist of leaveDoorId: " + dist[leaveDoorId]);
                                prev.set(leaveDoorId, new ArrayList<Integer>(Arrays.asList(nextParId, curDoorId)));
